...
Checked qualitatively (`scripts/check_tta.py`, `outputs/checkpoints/best_model.pt`, all 6 test images): plain vs. geometric-only TTA predictions agree on 98.97–99.62% of pixels per image; adding the 4 color views on top shifts a further 0.15–0.44% of pixels (geometric-TTA vs. geometric+color-TTA agreement: 99.56–99.88%). Both disagreement layers concentrate on roof-boundary pixels (mild edge refinement), not gross detections — expected given the model's already-confident interior predictions.

`notebooks/tta_experiment.ipynb` trains the full-data model (all 24 images, no held-out split, up to 100 epochs) and renders plain/geometric-TTA/geometric+color-TTA prediction grids on the 6 test images after every epoch, for qualitative inspection of how predictions (and TTA's effect on them) evolve over training. Since this run has no validation split, it has no automatic best-epoch signal — the quantitative best-epoch estimate (~18–21 of a 25-epoch budget) comes from the §3.5 CV ensemble comparison instead.
