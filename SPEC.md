...
**Test-time augmentation (optional, implemented):** `roof_seg/tta.py` provides `predict_with_tta()`, averaging sigmoid probability maps over the 8-element dihedral group (horizontal flip × 90° rotation) — the same transform family already verified mask-safe as an exact pixel permutation for training augmentation (§3.5). Each view's prediction is mapped back to original pixel coordinates exactly (no interpolation) before averaging. Checked qualitatively (`scripts/check_tta.py`, `outputs/checkpoints/best_model.pt`, all 6 test images): plain vs. TTA predictions agree on 98.97–99.62% of pixels per image; disagreement is concentrated on roof-boundary pixels (mild edge refinement), not gross detections — expected given the model's already-confident interior predictions.
