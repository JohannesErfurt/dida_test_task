...
class TrainConfig:
    ...
    would make them very noisy). `epochs=40` is a fixed budget rather than
    early stopping, because the final model trains on all 24 images with no
    validation split to early-stop against; the budget is chosen via CV.
    """
...
class TrainResult:
    ...
    def final_val_metrics(self) -> dict[str, float]:
        return self.val_metrics[-1] if self.val_metrics else {}
